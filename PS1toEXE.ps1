...
if( $cr.Errors.Count -gt 0 ) {
	...
	if( Test-Path $outputFile ) {
		...
		Write-Host $outputFile -ForegroundColor Red -NoNewlineparam([Parameter(Mandatory=$true)][string]$inputFile, [string]$outputFile=$null, [switch]$runtime20, [switch]$x86, [switch]$x64, [switch]$runtime30, [switch]$runtime40, [switch]$runtime50, [int]$lcid, [switch]$sta, [switch]$mta, [switch]$noConsole, [switch]$nested, [string]$iconFile=$null)
